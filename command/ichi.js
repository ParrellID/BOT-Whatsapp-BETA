...
case 'menu': case 'help': case '?': {
  let menu = `
Hallo 👋
Saya adalah BOT untuk Membantu Anda dengan Metode Whatsapp!
...
╚════════
  `
  let but = [
  {urlButton: {displayText: '➤ INSTAGRAMS',url: 'https://instagram.com/farxofc_'}}, 
  {urlButton: {displayText: '➤ WEBSITES',url: 'https://parrellid.vercel.app'}}, 
  {"quickReplyButton": {"displayText": "➤ DONATE","id": `donasi`},},
  {"quickReplyButton": {"displayText": "➤ OWNERS","id": "owner"},},
...
case 'bcgc': case 'bcgroup': {
  ...
  if (!text) throw `*ERROR-TEXT*\nText mana? Example : ${prefix + command} ${global.ownerName}`
...
case 'leave': {
  if (!isOwner) return m.reply(mess.botOwner)
  await ichi.groupLeave(m.chat).then((res) => m.reply('Mantaff! Beban Keluar Satu')).catch((err) => m.reply(fiturError()))
  }
  break
case 'block': {
  if (!isOwner) return m.reply(mess.botOwner)
...
case 'public': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
  ichi.public = true
  m.reply('☑️ | Sukses Mengubah Mode Bot Menjadi Publik')
  }
  break
case 'self': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
  ichi.self = false
  m.reply('☑️ | Sukses Mengubah Mode Bot Menjadi Self')
  }
  break
case 'eval': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.botOwner)
...
case 'kick': {
  ...
  if (!m.quoted && !text) return m.reply('❓| Siapa yang Mau di Kick ??')
...
case 'add': {
  ...
  if (!isBotAdmins) return m.reply(mess.botAdmin)
  if (!isAdmins) return m.reply(mess.admin)
  if (!m.quoted && !text) return m.reply('❓| Siapa yang Mau di Tambah ??')
  if (args[0].startsWith('08')) return m.reply('❗| Gunakan kode negara 62 Gan')
...
case 'promote': {
  ...
  if (!isBotAdmins) return m.reply(mess.botAdmin)
  if (!isAdmins) return m.reply(mess.admin)
  if (!m.quoted && !text) return reply('❓| Siapa yang Mau di Promote ??')
...
case 'demote': {
  ...
  if (!isBotAdmins) return m.reply(mess.botAdmin)
  if (!isAdmins) return m.reply(mess.admin)
  if (!m.quoted && !text) return reply('❓| Siapa yang Mau di Demote ??')
  if (args[0].startsWith('08')) return reply('❗| Gunakan kode negara 62 Gan')
  let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
...
case 'sticker': case 's': case 'stickergif': case 'sgif': {
  if (!quoted) return m.reply(`❗| Balas Vide / Image dengan Caption ${prefix + command}`)
  m.reply(mess.wait)
  if (/image/.test(mime)) {
